...
for(const viewport of viewports){
 ...
 const library=await page.evaluate(()=>{
  ...
  return{count:cards.length,columns:getComputedStyle(grid).gridTemplateColumns.split(' ').length,photoCards:cards.filter(x=>x.querySelector('.dg-real-photo-badge')).length,missingPhotos:cards.filter(x=>x.querySelector('.dg-photo-unavailable-v931')).length,overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,grid:{left:r.left,right:r.right,width:r.width},atlasComplete:DG_GYM_ATLAS_COMPLETE_V931,details:MACHINES.map(m=>({id:m.id,tip:m.tip,bestFor:m.bestFor,programUse:m.programUse,isCardio:m.isCardio}))};
 });
 if(library.count!==23||library.photoCards!==20||library.missingPhotos!==3||library.atlasComplete!==false)throw new Error(`${viewport.name}: expected 23 guides, 20 verified photos and 3 honest placeholders: ${JSON.stringify(library)}`);
 ...
 if(await page.locator('.dg-machine-card-v93').count()!==6)throw new Error(`${viewport.name}: cardio filter failed`);
